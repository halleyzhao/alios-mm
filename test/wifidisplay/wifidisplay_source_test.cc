...
#define MM_LOG_TAG  "WifiDisplaySouceTest"

class  WfdRemoteDisplay2;

class WfdSession2CB : public IWfdSessionCB
{
public:
    WfdSession2CB(WfdRemoteDisplay2* owner) : mOwner(owner) {}
    virtual ~WfdSession2CB() {}
...
private:
    WfdRemoteDisplay2* mOwner;
};

class  WfdRemoteDisplay2
{
public:
    WfdRemoteDisplay2( const char *iface )
    {
        DEBUG("create for iface [%s]", iface);

        mSessionListener = new WfdSession2CB(this);

        mSession = new WfdSourceSession(iface, mSessionListener);
...
private:
    WfdSourceSession *mSession;
    WfdSession2CB  *mSessionListener;
...
class WIFIDisplaySourceTest : public testing::Test
{
protected:
    virtual void SetUp()
    {
...
TEST_F(WIFIDisplaySourceTest, WFDSourceTest)
{
    char buf[128];
    WfdRemoteDisplay2 *source = NULL;
    MMSharedPtr<WfdRemoteDisplay2> wfdSource;

    sprintf(buf, "%s:%d", g_remote_ip, g_remote_port);
    source = new WfdRemoteDisplay2( buf );
    if (!source)
        return;

    wfdSource.reset(source);
    DEBUG("the wifidisplay source setup success\n");
    RunMainLoop();
    DEBUG("start to exit the wifidisplay source\n");
    wfdSource->dispose();
    wfdSource.reset();
...
_end:
    if (g_remote_ip)
        g_free(const_cast<char*>(g_remote_ip));
    return 0;
}
