...
MM_LOG_DEFINE_MODULE_NAME("BMPTest")
...
{
    uint32_t w,h;
    MMBufferSP data = BMP::decode(fname,
                                        1,
                                        1,
                                        outFormat,
                                        w,
    ...
    sprintf(name, "%s.save_d_%d_1_1.jpg", fname, outFormat);
    ...
    sprintf(name, "%s.save_d_%d_1_5.jpg", fname, outFormat);
    ...
    sprintf(name, "%s.save_d_%d_2_1.jpg", fname, outFormat);
    ret = RGB::saveAsJpeg(data,
                    outFormat,
                    w,
                    h,
...
int main(int argc, char * argv[])
{
    ...
    for ( int i = 0; i < TEST_FILES_COUNT; ++i ) {
        ...
        uint32_t jpegW, jpegH;
        mm_status_t ret = BMP::saveAsJpeg(TEST_FILES[i],
                        1,
                        1,
                        name,
                        jpegW,
