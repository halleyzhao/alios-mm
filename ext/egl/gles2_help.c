...
createShaders(const char *vertexShaderText, const char *fragShaderText, int texCount)
{
    ...
    glProgram = (GLProgram*)calloc(1, sizeof(GLProgram));
    if (!glProgram)
        return NULL;
...
EGLContextType *eglInit(Display *x11Display, XID x11Window, uint32_t fourcc, int isExternalTexture)
{
    ...
    context = (EGLContextType*)calloc(1, sizeof(EGLContextType));
    if (!context)
        return NULL;
    ...
    eglSurface = eglCreateWindowSurface(eglDisplay, eglConfig, x11Window, NULL);
